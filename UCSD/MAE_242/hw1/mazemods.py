...
def makePath(xI,xG,path,n,m,O):
    ...
    for l in range(L):
        gridpath[path[l][1]][path[l][0]] = [1.,1.,1-l/L] # white-->yellow
    return gridpath
...
# If any collisions, cost is 999999999, else cost is one for each action
...
# If any collisions, cost is 999999999, else cost is 2^(x[0]) for each action
...
# If any collisions, cost is 999999999, else cost is 2^(maxX - x[0]) for each action
